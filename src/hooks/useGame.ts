...
export function useGame(letterMode: number, gameMode: GameMode = 'daily') {
  ...
  useEffect(() => {
    try {
      ...
      const savedStats = localStorage.getItem(`letreco_stats_${letterMode}`);
      if (savedStats) {
      ...
      if (gameMode === 'daily') {
        const savedState = localStorage.getItem(`letreco_state_${letterMode}_daily_${dayNumber}`);
        if (savedState) {
  ...
  const saveStats = useCallback((s: GameStats) => {
    try {
      localStorage.setItem(`letreco_stats_${letterMode}`, JSON.stringify(s));
    } catch (error) {
