...
const MODES = [0, 4, 5, 6, 7] as const; // 0 = all
        ...
        {!isOnline ? (
          ...
          <>
            ...
            <div className="flex justify-center gap-2 mb-4">
              <button
                className={`btn btn-xs ${modeFilter === '' ? 'btn-primary' : 'btn-ghost'}`}
                onClick={() => setModeFilter('')}
              >
                Todos
              </button>
              <button
                className={`btn btn-xs ${modeFilter === 'daily' ? 'btn-primary' : 'btn-ghost'}`}
                onClick={() => setModeFilter('daily')}
              >
                ☀️ Diário
              </button>
              <button
                className={`btn btn-xs ${modeFilter === 'practice' ? 'btn-primary' : 'btn-ghost'}`}
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${
                        game.won ? 'bg-success/10' : 'bg-error/10'
                      }`}
                    >
